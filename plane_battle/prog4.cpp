...
void Plane::gun_shoot(Plane &plane, const int& REB_p, int status) noexcept { //status = 1 - пуля не долетела, 0 - долетела
    if (status) {
        return;
    }
...
void Plane::rocket_shoot(Plane &plane, const int& REB_p, int status) noexcept { //status = 1 - ракета не долетела, 0 - долетела
...
Link::Link(const Link &l):plane_count(l.plane_count), command(l.command), x(l.x), y(l.y) {
    ...
    for (int i = 0; i < 4; i++) {
        s[i] = l.s[i];
        rocketBar[i] = l.rocketBar[i];
        healthBar[i] = l.healthBar[i];
...
void Mission::gun_shoot_plane_in_enemy_t(const int& id1, const int& num1, const int& id2, const int& num2, int status) {
    ...
    p->gun_shoot(*enemy_p, REB_p, status);
    if (enemy_p->get_cur_health() <= 0) delete_plane_enemy_t(id2, num2);
}

void Mission::gun_shoot_plane_in_t(const int& id2, const int& num2,const int& id1, const int& num1, int status) {
    Plane *p = get_plane_t(id1, num1);
    if (!p) return;
    Plane *enemy_p = get_plane_enemy_t(id2, num2);
    if (!enemy_p) return;
    ...
    enemy_p->gun_shoot(*p, REB_p, status);
    if (p->get_cur_health() <= 0) delete_plane_t(id1, num1);
}

void Mission::rocket_shoot_plane_in_enemy_t(const int& id1, const int& num1, const int& id2, const int& num2, int status) {
    Plane *p = get_plane_t(id1, num1);
    if (!p) return;
    Plane *enemy_p = get_plane_enemy_t(id2, num2);
    if (!enemy_p) return;
    ...
    p->rocket_shoot(*enemy_p, REB_p, status);
    if (enemy_p->get_cur_health() <= 0) delete_plane_enemy_t(id2, num2);
}

void Mission::rocket_shoot_plane_in_t(const int& id2, const int& num2,const int& id1, const int& num1, int status) {
    Plane *p = get_plane_t(id1, num1);
    if (!p) return;
    Plane *enemy_p = get_plane_enemy_t(id2, num2);
    if (!enemy_p) return;
    ...
    enemy_p->rocket_shoot(*p, REB_p, status);
    if (p->get_cur_health() <= 0) delete_plane_t(id1, num1);
}
