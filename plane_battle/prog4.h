...
public:
    Gun():dmg(3), p(90), dist(300.f), rof(10) {}
    ~Gun() = default;
...
public:
    Rocket():dmg(100), dist(1000.f), rof(1), max_count(ROCKET_COUNT), cur_count(max_count) {}
    ~Rocket() = default;
    int get_dmg() const {return dmg;}
    float get_dist() const {return dist;}
    int get_rof() const {return rof;}
...
public:
    ...
    void gun_shoot(Plane &plane, const int& REB_p, int status) noexcept; //выстрелить из пушки
    void rocket_shoot(Plane &plane, const int& REB_p, int status) noexcept; //выстрелить ракетой
    int get_type() const {return type;}
    int get_max_health() const {return max_health;}
    int get_cur_health() const {return cur_health;}
    float get_speed() const {return speed;}
    int get_gun_count() const {return gun_count;}
    Rocket& get_rocket() {return rocket;}
    Gun& get_gun() {return gun;};
...
public:
    PRO():Plane(), cur_pro_count(2), max_pro_count(2) {type = 1;}
...
struct HealthBar {
    ...
    void update(int health) {
        float red_width = green.getLocalBounds().width;
        float offsetX = red_width * health/100;
        red_width = red_width * (100-health)/100;
        float height = green.getLocalBounds().height;
        red.setSize(sf::Vector2f(red_width, height));
        red.setPosition(green.getPosition().x + offsetX, green.getPosition().y);
    }
    HealthBar() {
        green.setFillColor(sf::Color(3,167,3));
        red.setFillColor(sf::Color::Red);
        green.setSize(sf::Vector2f(50.f, 9.f));
        update(100);
    }
};

struct RocketBar {
    sf::Sprite rocket[ROCKET_COUNT];
    int count;
    RocketBar():count(ROCKET_COUNT) {}
    void setPosition(float x, float y) {
        rocket[0].setPosition(x,y);
        rocket[0].setScale(7.f / rocket[0].getLocalBounds().width,
                           20.f / rocket[0].getLocalBounds().height);
        for (int i = 1; i < count; i++) {
            rocket[i].setScale(7.f / rocket[1].getLocalBounds().width,
                               20.f / rocket[1].getLocalBounds().height);
            rocket[i].setPosition(x += 9.f, y);
        }
    }
};
...
public:
    ...
    RocketBar rocketBar[4];
    void update_RocketBar() {
        for (int i = 0; i < plane_count; i++) {
            rocketBar[i].count = plane[i]->get_rocket().get_cur_count();
            int width = healthBar[i].green.getLocalBounds().width;
            float x_coord = healthBar[i].green.getPosition().x + width + 3;
            float y_coord = healthBar[i].green.getPosition().y;
            rocketBar[i].setPosition(x_coord, y_coord);
        }
    }
    void RocketBarLoadTexture(sf::Texture &texture) {
        for (int i = 0; i < plane_count; i++) {
            for (int j = 0; j < rocketBar[i].count; j++)
            rocketBar[i].rocket[j].setTexture(texture);
        }
    }
    void update_healthBar() {
        for (int i = 0; i < plane_count; i++) {
            int health = plane[i]->get_cur_health();
            healthBar[i].green.setPosition(s[i].getPosition().x-25, s[i].getPosition().y-40);
            healthBar[i].update(health);
        }
    }
    bool isSelectToMove;
...
public:
    ...
    void gun_shoot_plane_in_enemy_t(const int& id1, const int& num1, const int& id2, const int& num2,  int status);
    void gun_shoot_plane_in_t(const int& id2, const int& num2,const int& id1, const int& num1,  int status);
    void rocket_shoot_plane_in_enemy_t(const int& id1, const int& num1, const int& id2, const int& num2,  int status);
    void rocket_shoot_plane_in_t(const int& id2, const int& num2,const int& id1, const int& num1,  int status);
